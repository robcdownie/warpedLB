...
export function travelMinutes(
  ...
  if (!a || !b || a.id === b.id) {
    return { minutes: 0, approximate: true, source: 'estimated' };
  }
  const override = overrides.get(pairKey(a.id, b.id));
  if (override) {
    return { minutes: Math.max(0, override.minutes), approximate: true, source: 'override' };
  }
  const base = baseMinutes(a, b);
  const minutes = Math.max(1, Math.round(base * CROWD_MULTIPLIER[crowd]));
  return { minutes, approximate: true, source: 'estimated' };
}
